...
def process_row_top_features(row_data):
    ...
    # 6. perplexity_diff (signed difference)
...
def process_row_pairwise(row_data):
    """Process single row for pairwise features extraction (for multiprocessing)."""
    text1, text2 = row_data
...
def extract_pairwise_features(df: pd.DataFrame, n_jobs: int = None) -> np.ndarray:
    """Extract features comparing file_1 vs file_2 directly with multiprocessing."""
    if n_jobs is None:
    ...
    print(f"Using {n_jobs} cores for pairwise features extraction...")
    
    if len(row_data) < 100 or n_jobs == 1:
        # For small datasets, use single process
        features = []
        for data in tqdm(row_data, desc="Extracting pairwise features (single-threaded)"):
            features.append(process_row_pairwise(data))
    else:
        # Use multiprocessing for larger datasets
        with Pool(n_jobs) as pool:
            features = list(tqdm(
                pool.imap(process_row_pairwise, row_data),
                total=len(row_data),
                desc="Extracting pairwise features (multi-threaded)"
            ))
...
def prepare_data_for_model(
    ...
    # 3. Extract statistical features (existing) - lightweight, no multiprocessing needed
    ...
    # 5. Extract pairwise features (existing) - with multiprocessing
    print("Step 5: Extracting pairwise features...")
    pairwise_features = extract_pairwise_features(df, n_jobs=n_jobs)
