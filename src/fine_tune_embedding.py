...
class CustomEvaluator:
    ...
    def __call__(self, model, output_path=None, epoch=-1, steps=-1):
        ...
        # Calculate accuracy
        accuracy = sum(1 for pred, true in zip(predictions, self.labels) if pred == true) / len(self.labels)
        
        print(f"{self.name} accuracy: {accuracy:.4f}")
        return accuracy


def create_training_examples(textclf_df, max_positive_pairs=None, use_all_combinations=False):
    """
    ...
    # Validate input data
    if len(real_texts) < 2:
        raise ValueError(f"Cần ít nhất 2 real texts để tạo positive pairs, nhưng chỉ có {len(real_texts)}")
    if len(fake_texts) < 1:
        raise ValueError(f"Cần ít nhất 1 fake text để tạo negative pairs, nhưng chỉ có {len(fake_texts)}")
    ...
    num_negative_pairs = len([ex for ex in train_examples if ex.label == 1.0])  # Balance với positive pairs
    num_negative_pairs = min(num_negative_pairs, min_len)
    
    for i in range(num_negative_pairs):
        train_examples.append(
            InputExample(texts=[real_texts[i % len(real_texts)], fake_texts[i % len(fake_texts)]], label=0.0)
        )
...
def fine_tune_embedding(
    ...
    train_sentences1 = [example.texts[0] for example in train_examples[:200]]  # Sample để nhanh
...
if __name__ == "__main__":
    ...
    model = fine_tune_embedding(
        ...
        batch_size=16,
        output_path=f"{base_model_name}_case{case}_fine_tuned",
